...
{	//function that get the user input from search.php, and return URL for restAPI
	//substitute : chars have to be replaced with its hexadecimal ASCII code for using API
 	//pre_query : restAPI setting URL before query
	//searchString : the user input
	//query_len : expected length of current URL
	//query : current URL
	...
	int query_len = (strlen(pre_query) + strlen(searchString) + 1);
